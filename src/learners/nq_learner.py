...
class NQLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        # ->
        avail_actions_1 = batch["avail_actions_1"]
        avail_actions_2 = batch["avail_actions_2"]
        ...
        # ->
        chosen_action_qvals_1 = th.gather(mac_out_1[:, :-1], dim=3, index=actions_1).squeeze(3)  # Remove the last dim
        chosen_action_qvals_2 = th.gather(mac_out_2[:, :-1], dim=3, index=actions_2).squeeze(3)  # Remove the last dim
        ...
        with th.no_grad():
            ...
            for t in range(batch.max_seq_length):
                ...
                target_agent_outs_1, target_agent_outs_2 = self.target_mac.forward(batch, t=t)
            ...
            mac_out_detach_1[avail_actions_1 == 0] = -9999999
            mac_out_detach_2 = mac_out_2.clone().detach()
            mac_out_detach_2[avail_actions_2 == 0] = -9999999

            # cur_max_actions = mac_out_detach.max(dim=3, keepdim=True)[1]
            # target_max_qvals = th.gather(target_mac_out, 3, cur_max_actions).squeeze(3)
            cur_max_actions_1 = mac_out_detach_1.max(dim=3, keepdim=True)[1]
            target_max_qvals_1 = th.gather(target_mac_out_1, 3, cur_max_actions_1).squeeze(3)
            cur_max_actions_2 = mac_out_detach_2.max(dim=3, keepdim=True)[1]
            ...
            # ->
            target_max_qvals_1 = self.target_mixer(target_max_qvals_1, batch["state"])
            target_max_qvals_2 = self.target_mixer(target_max_qvals_2, batch["state"])
            ...
            if getattr(self.args, 'q_lambda', False):
                ...
                qvals_1 = th.gather(target_mac_out_1, 3, batch["actions_1"]).squeeze(3)
                qvals_1 = self.target_mixer(qvals_1, batch["state"])
                targets_1 = build_q_lambda_targets(rewards, terminated, mask, target_max_qvals_1, qvals_1,
                                    self.args.gamma, self.args.td_lambda)

                qvals_2 = th.gather(target_mac_out_2, 3, batch["actions_2"]).squeeze(3)
                qvals_2 = self.target_mixer(qvals_2, batch["state"])
                targets_2 = build_q_lambda_targets(rewards, terminated, mask, target_max_qvals_2, qvals_2,
                                                   self.args.gamma, self.args.td_lambda)

            else:
                # targets = build_td_lambda_targets(rewards, terminated, mask, target_max_qvals,
                #                                     self.args.n_agents, self.args.gamma, self.args.td_lambda)
                targets_1 = build_td_lambda_targets(rewards, terminated, mask, target_max_qvals_1,
                                                  self.args.n_agents, self.args.gamma, self.args.td_lambda)
                targets_2 = build_td_lambda_targets(rewards, terminated, mask, target_max_qvals_2,
                                                    self.args.n_agents, self.args.gamma, self.args.td_lambda)
        ...
        td_error = td_error_1 + td_error_2
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("q_taken_mean_1", (chosen_action_qvals_1 * mask).sum().item()/(mask_elems * self.args.n_agents), t_env)
            self.logger.log_stat("target_mean_1", (targets_1 * mask).sum().item()/(mask_elems * self.args.n_agents), t_env)
            self.logger.log_stat("q_taken_mean_2", (chosen_action_qvals_1 * mask).sum().item() / (mask_elems * self.args.n_agents), t_env)
            self.logger.log_stat("target_mean_2", (targets_1 * mask).sum().item() / (mask_elems * self.args.n_agents),
                                 t_env)
