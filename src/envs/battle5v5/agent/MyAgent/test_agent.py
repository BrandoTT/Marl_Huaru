...
from ...agent.MyAgent.test_utils import Trans
import copy
import random
seted = False
"""
...
class TestAgent(Agent):
    ...
    def __init__(self, name, config):
        ...
        :param config:阵营配置信息
        """
        super(TestAgent, self).__init__(name, config["side"])
    ...
    def my_cmd(self, sim_time, obs_side, cmd_list):
        self.process_observation(obs_side)  # 获取态势信息,对态势信息进行处理

        for plane in self.my_allplane_infos:
            approaching_missile = self.is_missile_approaching(self.enemy_missile_infos, plane)
            if len(approaching_missile) > 0:
                target_dir = {'X': 0, 'Y': 0, 'Z': 0}
                if approaching_missile['ver_dir'] == 'F':
                    # print('forward')
                    target_dir = BaseTSVector3.plus(target_dir, {'X': -1, 'Y': 0, 'Z': 0})
                else:
                    # print('backward')
                    target_dir = BaseTSVector3.plus(target_dir, {'X': 1, 'Y': 0, 'Z': 0})
                if approaching_missile['hor_dir'] == 'L':
                    # print('left')
                    target_dir = BaseTSVector3.plus(target_dir, {'X': 0, 'Y': 1, 'Z': 0})
                else:
                    # print('right')
                    target_dir = BaseTSVector3.plus(target_dir, {'X': 0, 'Y': -1, 'Z': 0})
                target_dir = Trans.body_to_ecef(plane['Heading'], plane['Pitch'], plane['Roll'], target_dir)
                target_dir = BaseTSVector3.normalize(target_dir)
                target_dir = BaseTSVector3.multscalar(target_dir, 5000)
                own_ecef = Trans.llh_to_ecef(plane['Lat'], plane['Lon'], plane['Alt'])
                target_pos = BaseTSVector3.plus(own_ecef, target_dir)
                if plane["Type"] == 1:
                    if target_pos['Z'] < 2000 or target_pos['Z'] > 15000:
                        target_pos['Z'] = 10000
                elif plane["Type"] == 2:
                    if target_pos['Z'] < 2000 or target_pos['Z'] > 10000:
                        target_pos['Z'] = 10000
                # target_pos['Z'] = plane['Z']
                leader_fire_route_list = [target_pos]  # 获取一个路径点,将敌方有人机的所在点设置为目标点
                data = self.get_move_data(plane)
                # EnvCmd.make_linepatrolparam 指令，可以令飞机沿给定路线机动，参数依次为 飞机ID，路线，速度，加速度，超载（与飞机转向角度有关）
                cmd_list.append(
                    CmdEnv.make_linepatrolparam(plane['ID'], leader_fire_route_list, data['move_max_speed'],
    ...
    def step(self, sim_time, obs_side, **kwargs) -> List[dict]:
        ...
        self.update_attack_handle_enemy(obs_side)

        self.process_decision(sim_time, obs_side, cmd_list)  # 调用决策函数进行决策判断

        self.my_cmd(sim_time, obs_side, cmd_list)
        return cmd_list  # 返回决策完毕的任务指令列表

    def update_attack_handle_enemy(self, obs_side):
        missileinfos = obs_side['missileinfos']
        for missile in missileinfos:
            if not missile['Availability'] \
                    and missile['LauncherID'] in self.attack_handle_enemy.keys():
                self.attack_handle_enemy.pop(missile['LauncherID'])
    ...
    def process_attack(self, sim_time, cmd_list):
        ...
        for plane_info in self.my_allplane_infos:  # 遍历所有的己方飞机，选择离己方最近距离的飞机进行攻击
            if plane_info['LeftWeapon'] == 0 \
                    or int(plane_info['ID']) in self.attack_handle_enemy.keys():  # 判断飞机是否还有导弹  LeftWeapon 飞机的导弹数量
                continue
            close_target = self.select_target(plane_info, self.enemy_allplane_infos)  # 选取一个里当前己方飞机最近的敌方飞机
            if len(close_target) < 1:  # 如果没有最近的敌方飞机
                continue
            if close_target['close_distance'] <= 60000:  # 当最近的目标离己方飞机的间距小于60000时
    ...
    def is_missile_approaching(self, missile_list, own_plane) -> {}:
        """
            选择离己方飞机最近的敌方missile
            :param own_plane:己方飞机信息
            :param enemy_list:敌方飞机信息列表
            :return:最近的敌方飞机信息
        """
        min_distance = 30000  # 超过此距离的敌方飞机不在考虑之内
        close_enemy = {}
        for enemy in missile_list:
            if enemy['EngageTargetID'] != own_plane['ID']:
                continue
            enemy_ecef = Trans.llh_to_ecef(enemy['Lat'], enemy['Lon'], enemy['Alt'])
            own_ecef = Trans.llh_to_ecef(own_plane['Lat'], own_plane['Lon'], own_plane['Alt'])
            distance = TSVector3.distance(enemy_ecef, own_ecef)  # 计算空间两点距离
            ...
            if distance < min_distance:  # 计算出一个离得最近的敌方实体
                ...
                dir_ecef = BaseTSVector3.minus(enemy_ecef, own_ecef)
                dir_body = Trans.ecef_to_body(own_plane['Heading'], own_plane['Pitch'], own_plane['Roll'], dir_ecef)

                if dir_body['X'] < 0:
                    close_enemy['ver_dir'] = 'B'
                else:
                    close_enemy['ver_dir'] = 'F'

                if dir_body['Y'] > 0:
                    close_enemy['hor_dir'] = 'R'
                else:
                    close_enemy['hor_dir'] = 'L'
        #
        if len(close_enemy) != 0 \
                and close_enemy['close_distance'] < 10000:
            if close_enemy['ver_dir'] == 'B':
                close_enemy['ver_dir'] = 'F'
            else:
                close_enemy['ver_dir'] = 'B'
            if close_enemy['hor_dir'] == 'R':
                close_enemy['hor_dir'] = 'L'
            else:
                close_enemy['hor_dir'] = 'R'
        return close_enemy  # 返回最近的敌方飞机信息
