...
class EnvRunner(XSimEnv):
    ...
    def get_action(self, obs):
        ...
        for side, agent in self.agents.items():
            try:
                cmd_list = self._agent_step(agent, cur_time, obs[side]) + cmd_list
            except:
                print("%s error!" % (side))
                cmd_list = []
            # print(cmd_list)
    ...
    def _agent_step(self, agent, cur_time, obs_side):
        ...
        @change date:
        """
        # print(f'agent: {agent}, cur_time: {cur_time}, obs_side: {obs_side}')
    ...
    def get_done(self, obs):
        ...
        flag_no = False # 用于判断是否红方是占领中心区域时间长而胜利的
        # 时间超时，终止
        cur_time = obs["sim_time"]
