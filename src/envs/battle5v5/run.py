...
"""
from src.envs.battle5v5.config import ADDRESS, config, POOL_NUM, ISHOST, XSIM_NUM
...
class BattleRunnerSignal(EnvRunner):
    ...
    def run(self, num_episodes):
        map_start = "N" # input("是否已启动态势显示工具? Y or N")
            ...
            if map_start == "Y" or map_start == "y":
                for i in range(num_episodes):
                    ...
                    while True:
                        ...
                        #     break
                    # sleep(20)
                    print("共", i + 1, "局：   红方胜", battle_results[0], "局：   蓝方胜", battle_results[1],
                          "局：   平局", battle_results[2], "局")
                    self._end()
                    ...
                    while True:
                        ...
                        if done[0]:  # 对战结束后环境重置
                            ...
                            if (done[1] == 1 and done[2] == 0):
                                ...
                                print("第", i + 1, "局：   平  局")
                            break
                        #print(f'做出动作后，拿到第二帧的状态:{obsplatforminfos}')
                    print("共", i + 1, "局：   红方胜", battle_results[0], "局：   蓝方胜", battle_results[1],
                          "局：   平局", battle_results[2], "局")
                    self._reset()
                    print(i, "重置")
        ...
        for i in range(num_episodes):
            ...
            while True:
                ...
                if done[0]:  # 对战结束后环境重置
                    ...
                    break
                obs = self.run_env(obs)
            print("共", i + 1, "局：   红方胜", battle_results[0], "局：   蓝方胜", battle_results[1], "局：   平局",
                  battle_results[2], "局")
            self._reset()
            print(i + 1, "重置")
        return battle_results
